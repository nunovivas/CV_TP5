...
def main():
    ...
    congratulations_display_duration = (
        40  # Number of loops to display the congratulations image
    )
    ...
    file_list = sorted(
        file for file in os.listdir(directory_path) if not file.startswith(".")
    )
    ...
    for file_name in file_list:
        ...
        )  # guarantees it has the original alpha channel
    ...
    while True:
        ...
        if results2.left_hand_landmarks:  # can only play with the left hand for now
            ...
            middle_coordinates = (
                int(middle_tip.x * frame.shape[1]),
                int(middle_tip.y * frame.shape[0]),
            )
            ring_coordinates = (
                int(ring_tip.x * frame.shape[1]),
                int(ring_tip.y * frame.shape[0]),
            )
            ...
            cv2.circle(
                frame, thumb_coordinates, 10, (0, 255, 0), -1
            )  # Green circle for thumb
            cv2.circle(
                frame, index_coordinates, 10, (0, 0, 255), -1
            )  # Red circle for index finger
            cv2.circle(
                frame, middle_coordinates, 10, (255, 0, 0), -1
            )  # Blue circle for middle finger
            cv2.circle(
                frame, ring_coordinates, 10, (255, 255, 0), -1
            )  # Yellow circle for ring finger

            # Check if the distance is below the threshold to detect a pinch
            if isPinch:
                # Draw "Pinch Detected" in the far right corner
                if f.isPinchInsideBox(
                    bb_box,
                    thumb_coordinates,
                    index_coordinates,
                    middle_coordinates,
            ...
            if index_coordinates and pinchInside:
                ...
                bb_box = (
                    index_coordinates[0] - int(bb_box_size[0] / 2),
                    index_coordinates[1] - int(bb_box_size[1] / 2),
                    *bb_box_size,
                )
        ...
        fpsCalculations = f.drawFps(
            t_start, frame, frame_index, last_reset_time
        )  # must return the frame index
        ...
        if frame_index < len(framesWithBasketball):
            ...
            img_to_display = cv2.resize(
                img_to_display, (bb_box_size[0], bb_box_size[1])
            )
        ...
        # BASKET
        # Resize the image to fit inside the box
        basket_to_display = cv2.resize(
            basket_image, (basket_box_size[0], basket_box_size[1])
        )
        ...
        if distance_between_centers < score_distance and not show_congratulations:
            ...
            while int(distance_between_centers) < int(minimum_distance_for_spawn):
                ...
                print(
                    f"BEFORE:Distance between centers:{distance_between_centers} minimum:{minimum_distance_for_spawn}"
                )
                bb_box = f.shuffleBox(mainFrameWidth, mainFrameHeight, bb_box_size)
                distance_between_centers = f.center_distance(bb_box, basket_box)
                print(
                    f"AFTER:Distance between centers:{distance_between_centers} minimum:{minimum_distance_for_spawn}"
                )
